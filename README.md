...
- **Typed resolution** (v2.4): `container.get(UserService)` is inferred as `UserService`, not `Any` — IDE autocomplete and type-checkers resolve the component.
...
**Scope LRU Eviction has been removed** to guarantee data integrity.
  ...
  * **Manual usage (split lifecycle):** when activate and deactivate happen in separate calls (ASGI middleware and the like), call `container.cleanup_scope("scope_name", scope_id)` yourself when the context ends to prevent memory leaks. *(Public since v2.4.1.)*
