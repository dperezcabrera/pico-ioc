...
### Scope Lifecycle & Memory Management (v2.1.3+)

Unlike simple caches, `pico-ioc` treats scopes (like `request` or `session`) as **lifecycle registries**.

  * **Persistence:** Once a scope ID is activated (e.g., a Request ID), objects created within it persist indefinitely until explicitly cleaned up. There is **no automatic LRU eviction**. This ensures that under high concurrency (e.g., 10k connected websockets), active users are never arbitrarily disconnected or erased from memory to make room for new ones.
  * **Cleanup Responsibility:**
      * If you use **framework integrations** (like `pico-fastapi`), cleanup is handled automatically by the middleware.
      * If you manage scopes **manually** (e.g., in a custom background worker), you **must** clean them up to prevent memory leaks:
...
scope_id = "task-123"
token = container.activate_scope("request", scope_id)
try:
    # ... do work ...
...
### `@cleanup`
  ...
  - Purpose: Marks a method to be called when the container is shut down (via `container.shutdown()` or `await container.ashutdown()`).
...
class ConnectionPool:
    ...
    @cleanup  # Called by container.ashutdown()
...
async def main():
    ...
    print("Shutting down...")
    await container.ashutdown()  # Async shutdown calls all @cleanup methods
...
In these cases, you can defer its creation until it's actually requested for the first time using `lazy=True`.
  ...
  - Behavior:
      ...
      - **Async Support (v2.1.3+):** If a lazy component requires asynchronous initialization (e.g., `async def @configure`), you **must** use `await container.aget(Component)` to resolve it. Using `get()` will raise an error. `aget()` will automatically detect the proxy and hydrate it asynchronously.
